...
public:
  ...
  explicit WireChamberEventFragment(artdaq::Fragment const &f)
      : artdaq_fragment_(f) {}

  static WireChamberEventMetadata
  generateMetadata(std::string hostname,
                   WireChamberFragment::ControllerHeader header) {
  ...
  WireChamberEventMetadata const *metadata() const {
    if (!artdaq_fragment_.hasMetadata()) {
      auto md = generateMetadata("UNKNOWN MWPC",
                                 WireChamberFragment::ControllerHeader());
