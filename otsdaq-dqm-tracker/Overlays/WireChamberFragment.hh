...
public:
  ...
  explicit WireChamberFragment(artdaq::Fragment const &f)
      : artdaq_fragment_(f) {}
  ...
  WireChamberMetadata generateMetadata(std::string hostname) const {
    ...
    output.TDCCount = 0;
    // For validation
    size_t expectedWordCount = ControllerWordCount(GetControllerHeader()) -
                               sizeof(ControllerHeader) / 2;
    ...
    if (expectedWordCount != tdcWordCount) {
      std::cerr << "ERROR: TDC Headers report different size than the "
                   "controller header!"
                << std::endl
                << "Controller Header: " << std::to_string(expectedWordCount)
                << std::endl
                << "TDC Headers: " << std::to_string(tdcWordCount) << std::endl
                << "Fragment Size: "
                << std::to_string(artdaq_fragment_.dataSizeBytes() / 2)
                << std::endl;
    }
  ...
  ControllerHeader const *GetControllerHeader() const {
    return reinterpret_cast<ControllerHeader const *>(
  ...
  TDCEvent const *dataBegin() const {
    return metadata()->EventCount > 0 ? reinterpret_cast<TDCEvent const *>(
                                            GetTDCHeaders() + tdcHeaderCount())
                                      : nullptr;
  }
  ...
  bool validateEvent(TDCEvent const *evt) const {
    ...
    if (evt->controllerEventTimestampPad != 0 || evt->eventStatusPad != 0 ||
        evt->tdcNumberPad != 0 || evt->triggerTypePad != 0 ||
        evt->wordCountPad != 0) {
      validEvent = false;
      ...
      TRACE(
          ...
          "FFFF FFFF 000F 0FFF");
      TRACE(4, "WCF::validateEvent: Data: %x %x %x %x %x %x %x", word1, word2,
  ...
  TDCEvent const *getValidEvent(TDCEvent const *evt) const {
    ...
    TRACE(4, "getValidEvent: end=%p, bytePtr=%p, evt=%p", (void *)end,
          (void *)bytePtr, (void *)evt);
  ...
  TDCEvent const *nextEvent(TDCEvent const *evt) const {
    auto end =
        reinterpret_cast<uint16_t const *>(artdaq_fragment_.dataEndBytes());
